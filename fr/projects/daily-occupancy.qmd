...
// encore atteintes.
//
// La priorité est établie ici, au moment du rendu, plutôt qu'en réécrivant
// les lignes archivées. Lorsque la Ville publiera enfin les dates manquantes,
// celles-ci quitteront contingencyDates à la construction suivante et le
// tableau reviendra de lui-même aux données ouvertes.
...
latestAudit = audit_history.records.find(
  record => record.date === audit_history.coverage.last_audit_date
)
latestEmergencyEmptyDifference = latestAudit.mismatches.find(
  mismatch => mismatch.key === "emerg_total" && mismatch.column === "unoccupied"
)
```
...
::: {.bq-status-note}
**Il s'agit du dernier relevé publié, et non de la disponibilité des lits en
temps réel.** Une place indiquée comme libre dans le fichier peut déjà être
occupée, indisponible ou ne pas convenir à la personne qui appelle. Utilisez
les coordonnées vérifiées ci-dessous si une personne a besoin d'une place
maintenant.
:::
...
html`<div class="bq-audit-alert" role="alert">
  ...
  de la Ville ne correspondait pas à ses propres fichiers publics lors de
  ...
  du ${auditFirstLabel} au ${auditLastLabel}. Le ${auditLastLabel}, le tableau de
  bord montrait <strong>${Math.abs(latestEmergencyEmptyDifference.delta)} places
  vides de plus dans les refuges d'urgence</strong> que les données publiques.
  <a href="data-validation.html">Voir les différences →</a></p>
</div>`
```

```{=html}
<nav class="bq-purpose-grid bq-purpose-grid-single" aria-label="Obtenir de l'aide maintenant">
...
```{ojs}
//| echo: false
// Rattaché à latestOpenDate, et non à latestDate. Ces vérifications valident
// l'agrégation des données ouvertes; un jour de recours n'en contient aucune
// et chaque assertion échouerait pour la mauvaise raison.
...
```{ojs}
//| echo: false
// Indique au lecteur laquelle des deux sources a produit la date affichée.
sourceNote = {
  ...
  return html`<div class="bq-warn" style="margin:0.5rem 0 1rem;font-size:0.88em">
    <p style="margin:0">
      ...
      cette date provient donc de sa <a href="${CITY_CENSUS_URL}"
      target="_blank">page Web quotidienne</a>. Les personnes et les lits
      occupés correspondent aux données ouvertes. Les places libres, les places
      ouvertes et le taux d'occupation sont surestimés, car la page Web compte
      comme ouverts des lits hors service. La colonne « Financé mais
...
// données ouvertes, afin qu'un jour de recours s'affiche exactement comme les
// autres au lieu d'exposer les libellés bruts de la Ville.
rowTemplate = new Map(
