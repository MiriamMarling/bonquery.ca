...
// The open data file is authoritative and carries funded capacity, so it is
// always preferred. The City's daily webpage is the fallback, and is used only for
// dates where no open data has been published yet.
//
...
::: {.bq-status-note}
**This is the latest published record, not live bed availability.** A space
shown as empty in the file may already be used, unavailable, or not suitable for
the person calling. Use the verified contacts below when someone needs a place
now.
:::
...
html`<div class="bq-audit-alert" role="alert">
  ...
  from ${auditFirstLabel} to ${auditLastLabel}. On ${auditLastLabel}, the dashboard
  showed <strong>${Math.abs(latestEmergencyEmptyDifference.delta)} more empty emergency beds</strong>
  than the public data showed. <a href="data-validation.html">See what differed →</a></p>
</div>`
```
...
sourceNote = {
  ...
  return html`<div class="bq-warn" style="margin:0.5rem 0 1rem;font-size:0.88em">
    <p style="margin:0">
      ...
      webpage</a>. People and beds used match the open data. Empty, open to
      use, and percent full run high, because the webpage counts offline beds
      as open. Funded but unavailable stays blank until the open data lands.
    </p>
  </div>`
}
```
