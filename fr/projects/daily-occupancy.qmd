...
description: >
  Dernières données publiées par la Ville sur l'utilisation et la capacité des
  refuges de Toronto. Il ne s'agit pas de la disponibilité en temps réel.
---
...
Ce tableau réunit deux sources de la Ville. Le **sélecteur de date** ci-dessous
permet de consulter toute date depuis janvier 2021 jusqu'à aujourd'hui; la page
...
de la Ville n'affiche que sa date la plus récente. Lorsque le fichier détaillé
de données ouvertes est retardé, BonQuery utilise temporairement les lignes
plus récentes de la page Web et en indique clairement la source. La colonne
**Hors service** reste vide pour ces dates jusqu'à l'arrivée du fichier
détaillé.

::: {.bq-status-note}
**N'utilisez aucun des deux tableaux de la Ville comme indicateur de la
disponibilité des lits en temps réel.** Il s'agit d'instantanés pris à 4 h, et
non d'une liste des places disponibles maintenant. Une place indiquée comme
libre peut déjà être occupée, indisponible ou ne pas convenir à la personne qui
appelle. Utilisez les coordonnées vérifiées ci-dessous si une personne a besoin
d'une place maintenant.
:::
...
latestDate > latestOpenDate
  ? html`<div class="bq-source-note" role="status">
      <p><strong>Les deux sources de la Ville ont des dates de publication
      différentes.</strong> Le fichier détaillé de données ouvertes sur les
      refuges va actuellement jusqu'au
      <strong>${fmtDate(latestOpenDate)}</strong>; la page Web quotidienne de la
      Ville va jusqu'au <strong>${fmtDate(latestDate)}</strong>. BonQuery utilise
      temporairement la page Web plus récente lorsque c'est possible et indique
      clairement la source.</p>
    </div>`
  : html``
```
...
La page Web et le fichier de données ouvertes peuvent être publiés selon des
calendriers différents. [Voir comment BonQuery combine et étiquette les deux
